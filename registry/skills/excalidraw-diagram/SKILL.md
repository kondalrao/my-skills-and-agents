...
description: Use when creating Excalidraw .excalidraw diagrams from natural language,
  workflows, architectures, systems, flowcharts, mind maps, data flows, or visual
  explanations.
---

# Excalidraw Diagram Creator and Generator

Generate `.excalidraw` JSON files from natural-language requests, technical notes, workflows, architectures, and concepts. The output should **argue visually**, not just display information.

**Setup:** If the user asks you to set up this skill (renderer, dependencies, etc.), see `README.md` for instructions.

## When to Use

Use this skill when the user asks to create, draw, visualize, diagram, map, or generate an Excalidraw file for:

- Process flows, workflows, procedures, and decision trees
- System architectures, components, modules, dependencies, and data flows
- Relationship diagrams, entity relationships, ER-style models, and object/class structures
- Mind maps, concept hierarchies, and idea breakdowns
- Sequence diagrams, timelines, swimlanes, and cross-role business flows

For simple requests, extract the entities, relationships, and flow directly. For technical or educational diagrams, follow the deeper research, evidence-artifact, and render-validation rules below.

**Related skill:** Use `markdown-mermaid-writing` instead when the output should remain Markdown with Mermaid diagrams as the version-controlled source. Keep this skill for `.excalidraw` canvases and visual argument diagrams.

## Natural-Language Request Mapping

| User asks for | Produce | Key extraction work |
|---------------|---------|---------------------|
| Workflow, process, steps, procedure | Flowchart | Start/end, actions, decisions, branches |
| Architecture, system, modules, components | Architecture diagram | Boundaries, services, data/control flow, protocols |
| Relationship, dependencies, structure | Relationship diagram | Nodes, typed edges, direction, labels |
| Mind map, concepts, breakdown | Mind map or tree | Central topic, branches, subtopics |
| Data flow, data processing, transformation | Data flow diagram | Sources, stores, transforms, destinations |
| Swimlane, actors, responsibilities | Business flow | Lanes, handoffs, owner per activity |
| Class, inheritance, OOP model | Class diagram | Classes, fields, methods, relationship types |
| Sequence, interaction, messages | Sequence/timeline | Actors, lifelines, calls, returns, time order |
| Database, entity, schema | ER diagram | Entities, keys, cardinality, junction tables |

If the request is too large for one readable diagram, create a high-level diagram first and propose focused follow-up diagrams for crowded subsystems.
...
Additional preserved generator assets live in:

- `templates/` for starter flowchart, relationship, mind map, data-flow, swimlane, class, sequence, and ER diagrams
- `scripts/` for optional Excalidraw library/icon helpers
- `references/generator-excalidraw-schema.md` and `references/generator-element-types.md` for the former generator reference material
...
```bash
cd /Users/kkomarag/.agents/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd /Users/kkomarag/.agents/skills/excalidraw-diagram/references
uv sync
