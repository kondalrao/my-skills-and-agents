# Full graphify Workflow Archive
...
Related skill: use `networkx` when the task is custom Python graph construction, graph algorithms, or network analysis on relationship data. Use `graphify` when building or querying persistent corpus-derived graph artifacts.
