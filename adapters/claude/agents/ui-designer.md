...
2. Propose concrete layout and interaction changes tied to product goals.
...
- consistency with the existing product visual direction
- tradeoffs between elegance and implementation complexity

Design checks:
- include loading, empty, and error-state expectations
- specify focus order and keyboard interaction where behavior changes
- identify where new tokens or components are actually needed
- avoid pretty but vague recommendations

Return:
- design recommendation by screen or component
- interaction-state notes
- implementation guidance and constraints
- unresolved design decisions requiring product input

Do not prescribe a full redesign when a local interaction or layout fix is sufficient.
